...
{
    // string coord_path_a = "/home/hbut-lichuang/桌面/test/a.yaml";
    // string coord_path = "/home/hbut-lichuang/桌面/test/a.yaml";
    // fstream fin;
    // fin.open(coord_path, ios::in);
    // string s;
    ...
    // cout<<s<<endl;
    // fin.close();
    // YAML::Node hh = YAML::Load(s);
    
    // YAML::Node config = YAML::LoadFile(coord_path);
    // std::cout<<config.size()<<"jfkdl"<<std::endl;
    // std::cout<<config["KE0200110076"]["max_iter"].as<string>()<<std::endl;
  // YAML::Node config = YAML::LoadFile(coord_path);
  // YAML::LoadAllFromFile(coord_path);
  // param_node = YAML::LoadFile(coord_path);
  // std::cout<<"coord_path:"<<coord_path<<std::endl;
  // std::cout<<"param_name:"<<param_name<<std::endl;

  
    cv::FileStorage fin(coord_path, cv::FileStorage::READ);
    // cv::FileStorage fin;
    // if(!fin.isOpened()){
  ...
  // //初始化弹道补偿参数
  // max_iter = param_node[param_name]["max_iter"].as<int>();
...
PnPInfo CoordSolver::pnp(const std::vector<Point2f>& points_pic, const Eigen::Matrix3d& rmat_imu, int method)
{
  ...
  if (points_pic.size() == 4)
  {
    ...
    if (apex_wh_ratio > armor_type_wh_thres)
      points_world = { { -0.1125, 0.027, 0 }, { -0.1125, -0.027, 0 }, { 0.1125, -0.027, 0 }, { 0.1125, 0.027, 0 } };
    else
      points_world = { { -0.066, 0.027, 0 }, { -0.066, -0.027, 0 }, { 0.066, -0.027, 0 }, { 0.066, 0.027, 0 } };
  }
  //长度为5进入大符模式
  else if (points_pic.size() == 5)
  {
    points_world = {
      { -0.1125, 0.027, 0 }, { -0.1125, -0.027, 0 }, { 0, -0.565, -0.05 }, { 0.1125, -0.027, 0 }, { 0.1125, 0.027, 0 }
    };
  }
  // 旋转矩阵cv
  Mat rvec;
