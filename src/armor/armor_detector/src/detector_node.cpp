...
/**
 * @brief 通过ros2参数初始化装甲板检测器，方便调参
 * @return std::unique_ptr<Detector> 装甲板检测器指针
 */
...
std::vector<Armor> DetectorNode::detectArmors(const sensor_msgs::msg::Image::ConstSharedPtr& img_msg)
{
	...
	if (debug_)
	{
		...
		if (!detector_->debug_preprocess_img.empty())
			preprocess_img_pub_.publish(cv_bridge::CvImage(img_msg->header, "mono8", detector_->debug_preprocess_img).toImageMsg());
	}
